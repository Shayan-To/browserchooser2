...
#End If
    ...
    Public Shared Sub DoUpdate()
        ...
        Try
            ...
            GeneralUtilities.LaunchUserMode(GeneralUtilities.ListOfCommands.ApplyUpdate, Globals.gStartupLauncher.URL, Application.ExecutablePath & ".new.exe", False)
    ...
    Private Shared Function askForUpdate() As Boolean
        ...
        If IsNothing(lPublicVersion.id) = False Then
            If lPublicVersion.id <> Globals.CurVersion Then
                'build an do you want to update dialog box
                Dim lResult As DialogResult
                Dim lOut As String = "An update to Browser Chooser 2 is available!" & Environment.NewLine &
                    "Do you want to download and apply this update?" & Environment.NewLine &
                    "Press Cancel to turn off update notification." & Environment.NewLine &
                    Environment.NewLine &
                    "Your version: " & Globals.CurVersion + Environment.NewLine &
                    "Available version: " & lPublicVersion.id & Environment.NewLine &
                    Environment.NewLine &
