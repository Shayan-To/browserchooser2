...
Public Class Settings
    ...
    Public Function [As](Of T)() As T
        '' from: http://stackoverflow.com/a/39543575
        Dim lType = GetType(T)
        Dim lOut = Activator.CreateInstance(lType)

        If lType.BaseType IsNot Nothing Then
            For Each lProp As Reflection.FieldInfo In lType.BaseType.GetFields()
                Debug.Print(lProp.Name)
                If lProp.IsPublic And lProp.IsStatic = False Then
                    lProp.SetValue(lOut, lProp.GetValue(Me))
                End If
            Next
        End If

        Return DirectCast(lOut, T)
    End Function
End Class
